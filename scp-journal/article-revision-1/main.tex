...
process $\Stop$, which deadlocks:~does not engage into any event or terminate
successfully. For an external choice $\Extchoice e: A @ e \then P(e),
A\subseteq \Sigma$ over a set $A$ of events, we use the abbreviation $e:A
\then P(e)$. An iterated internal choice is not defined for an empty index
set.

The branches of an external choice can be protected by guards. The process
$$
P(x_1,\dots,x_n) = \text{bexpr}_1(x_1,\dots,x_n) \& Q(x_1,\dots,x_n) \extchoice
\text{bexpr}_2(x_1,\dots,x_n) \& R(x_1,\dots,x_n)
$$
parametrised over $x_1,\dots,x_n$ branches according to Boolean expressions
$\text{bexpr}_1(x_1,\dots,x_n)$ and $\text{bexpr}_2(x_1,\dots,x_n)$ over
variables from $\{ x_1,\dots,x_n\}$: if $\text{bexpr}_1(x_1,\dots,x_n)$ evaluates to
$\ist$ and $\text{bexpr}_2(x_1,\dots,x_n)$ to $\isf$, $P$ behaves like $Q$.
If $\text{bexpr}_1(x_1,\dots,x_n)$ evaluates to
$\isf$ and $\text{bexpr}_2(x_1,\dots,x_n)$ to $\ist$, $P$ behaves like $R$. If both
...
$\Sigma-[P/s]^0$, where $[P/s]^0$ denotes the \emph{initials} of
$P/s$, that is, the events that $P/s$ may engage into. Nondeterministic behaviour in a given process
...
graph $G(P) = ( N, {\ii n}, \Sigma, t : N\times\Sigma \pfun N, r : N \fun \mathbb{P}\mathbb{P}(\Sigma))$. Let $V\subseteq\Sigma^*$ be a finite prefix-closed set of
sequences of events. Suppose that  $G(P)$ reaches $k < \card{N}$ nodes under $V$,
that is, $\card{t(\ii n,V)} = k$. Let $V.\Sigma$ denote the set of all sequences
...
refusals or minimal acceptances is disregarded in the product construction.
We consider two normalised transition graphs over the same alphabet $\Sigma$.
\[
...
\end{eqnarray}
%
The following lemma is used later in the proof of our main theorem.
%
...
\end{equation}
The collection $\minsub({\cal C})$ contains all those sets of ${\cal C}$ that
are not true supersets of other members of ${\cal C}$. With this definition,
the relation between failures and minimal hitting sets of minimal acceptances
...
a (not necessarily minimal) hitting set of $\minaccs(Q/s)$.
\end{enumerate}
\end{lemma}
\begin{proof}
...
(\ref{eq:failrefb}), there exists $s\in\trc(P)\cap \trc(Q)$ that satisfies
...
states and $G(Z)$ has $q=5$, so $pq-1=19$ is the index of the last test to be
executed  according to Theorem~\ref{th:failurestest}), tests $U_F(0),\dots,
...
\]
The test $U_F(4)$ offers hitting sets from $\minhits(3) = \{\{b\},\{c\}\}$ in
branch (\ref{eq:ufd}). Therefore,  there exists one test execution where
$Z/s$ accepts only $\{b\}$ due to the internal choice (note from
Fig.~\ref{fig:tgZ} that $G(Z)/s = \text{node}\ 4$), while $U_F(4)/s$ only
offers $\{c\}$ in branch (\ref{eq:ufd}) or $\{ a\} = \Sigma - [3]^0$ for
...
$\{c \}$, while $U_F(4)/s$ chooses to accept only $\{a,b\}$. Therefore,
$%\[
...
because of (\ref{eq:ifpaT}).  Now $\trc(Q)\subseteq\trc(P)$ implies
...
last step of the test execution. Therefore, the upper bound
...
the test $U_F(pq-1)$ and $U_T(pq-1)$, respectively. These tests fail, because
they allow for execution of the following trace of length 12
$$
s_\text{fail} = a.a.a.b.a.a.a.b.a.a.a.b
$$
Only in the very last step, the final $b$-event can be produced by $Q$ but is refused by $P$.
Therefore, tests $U_F(11)$ and $U_T(11)$ enter their first branch (\ref{eq:ufa}) and (\ref{eq:uta}), respectively,\ when engaging into the last $b$, and this produces the $\efail$-event and shows  that $P\not\lessdet_F Q$ and $P\not\lessdet_T Q$ according to
Theorem~\ref{th:failurestest} and Theorem~\ref{th:tracetest}.. The tests
$U_F(j)$ and $U_T(j)$ with $j < pq-1 = 11$ only execute true prefixes of $s_\text{fail}$ which
are allowed for $P$, so that these tests pass.
...
\bibitem{DBLP:conf/rssrail/PeleskaHH16}
...
\newblock \href {http://dx.doi.org/10.1007/978-3-319-33951-1_3}
  {\path{doi:10.1007/978-3-319-33951-1_3}}.
...
\newblock \href {http://dx.doi.org/10.1007/s10270-017-0595-8}
  {\path{doi:10.1007/s10270-017-0595-8}}.
