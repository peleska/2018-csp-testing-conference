...
to mark it with an inc event.} It should be noted, however, that it is
...
\fixme{Missing hiding. And also, if the SUT blocks, the test should block,
no?} obligation to check that {\it all} possible executions have been
...
failures refinement tests as well.} holds, which means that every possible
