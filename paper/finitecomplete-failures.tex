...
The specification of $U_F(p,s)$ implies that the test always stops after having engaged into a trace $s\in\trc(Q)$ of maximal length $p$ or $p+1$: 
...
and the execution stops without a PASS or FAIL event. Such an execution is also interpreted as FAIL.
...
Recall that $\minhits(P/s)=\varnothing$ implies  $\minaccs(P/s) = \{\varnothing\}$,
so branches (\ref{eq:ufa}) and (\ref{eq:ufb}) can be taken in this situation.
