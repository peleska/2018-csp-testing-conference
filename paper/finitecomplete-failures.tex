...
Alternatively, a faulty SUT $Q$ 
might internally deadlock after a trace $s$ where $\#s < j$ and
$\minhits(G(P)/s) \neq \varnothing$,
so that $(Q\parallel[\Sigma] U_F(j))/s$ deadlocks as well.
...
in branch~(\ref{eq:ufc}); this follows again from
...
test cases $U_F(j)$ with $0\le j < pq$. Therefore, it has to be shown that for any $s$ leading to an error
...
\fxnote{jp: this is a misunderstanding: we are not looking at the product graph of $Q$ and $U_F(j)$, but at the product graph of $Q$ and $P$. I have added more explanatory 
...
$U_F(\#u,\#u,G(P)/u)$. The guard condition $(k < j)$ for branch
