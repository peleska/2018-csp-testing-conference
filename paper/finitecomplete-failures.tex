...
$[G(P)/s]^0$ is {\it not} a hitting set of $\minhits(Q/s)$, because otherwise at least
...
is not an element of $\minhits(Q/s)$, also $H$ is not contained in $\minhits(Q/s)$.
Now this is a contradiction to Lemma~\ref{lemma:hseta}, since $Q\ conf\ P$ by assumption,
so $H$ should also be a (not necessarily minimal) hitting set in $\minhits(Q/s)$.
