...
A finite sequence of events $s\in\Sigma^*$ is a \emph{trace} of $P$, if and only if 
there
...
Exploiting formulas (\ref{eq:accref},\ref{eq:accinitials},\ref{eq:accsubset}),
...
\fxnote{jp: I have now introduced notation $\trc(G(P))$ above, so I think that the
statement of the lemma is now sufficiently clear.}
%
\begin{lemma}
...
\fxnote{jp: I have moved this lemma now to the section on preliminaries, this should make the main section on the finite complete test suite easier to read.}
...
\item $P\lessdet_F Q$ \fixme{Note that if traces refinement is already
    given, this is conf.}
    \fxnote{jp: sorry, I do not understand what you are saying here}
...
\fxnote{jp: Above Example 1, I have now introduced the notation $\minaccs(G(P)/s)$ and
$\maxrefs(G(P)/s)$.}
