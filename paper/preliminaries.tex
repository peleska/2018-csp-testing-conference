...
we do not need in the rest of the paper. We need (5) here to show that the set of
maximal refusals of $P/s$ equals $r(G(P/s))$.}

Given any non-diverging, non-terminating, finite-state process $P$,
...
contained in each minimal acceptance set.\fixme{alcc: the a from node 2 should go back to 0, I think.}
\fxnote{jp: No, this is correct: state 0 corresponds to $P$, 1 to $Q\intchoice R$,
2 to $P\intchoice R$, and 3 to $R$. 
Therefore, 2 goes with $a$ to $(P/a) = Q\intchoice R$, that is, to node 1. I think that
this is now explained sufficiently clear  in the example.}
