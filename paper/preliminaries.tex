...
collection of finite sets. To this end, the following definitions and results are 
useful\footnote{The authors are grateful to @todo for suggesting the applicability of Sperner's Theorem in the context of the work presented here.}. 
A \emph{Sperner Family} is a collection $S \subseteq 2^\Sigma$ of sets from a given universe $\Sigma$ that do not contain each other, that is, 
...
We observe that the collection of minimal hitting sets constructed from the minimal acceptances of a CSP process state build a Sperner Family. Therefore, the following theorem can be applied.
\begin{theorem}[Sperner's Theorem~\cite{sperner_satz_1928}]
\label{th:sperner}
The largest Sperner family $S$ 
over an $n$-element universe $\Sigma$ has cardinality 
$$
\card{S} = \binom{n}{\lfloor\frac{n}{2}\rfloor}.
$$
\xbox
\end{theorem}
