...
\fxnote{jp: I have clarified the introduction of acceptances and removed the statements
we do not need in the rest of the paper. We need (5) here show that the set of
maximal refusals of $P/s$ equals $r(G(P/s))$.}
...
\fxnote{jp: No, this is correct: state 0 corresponds to $P$, 1 to $Q\intchoice R$,
2 to $P\intchoice R$, and 3 to $R$. 
Therefore, 2 goes with $a$ to $(P/a) = Q\intchoice R$, that is, to node 1. }
...
it here. I suggest just to add a reference.} \fxnote{jp: done}Formula (\ref{eq:failconf}) has been
...
%\fxnote{jp: You are right: all states in G(P) are reachable by construction of the graph -- I have added this information to the section introducing the graphs.}
Let
...
\end{equation}
\fixme{You already used this above. Perhaps you want to move this to before
Lemma 4?}\fxnote{jp: done}
...
%We note that we use
%$\minhits(G(P)/s) = \minhits(P/s)$, as well $\minaccs(G(Q)/s) =
%\minaccs(Q/s)$, which follow from the identification of process states with
%states of the normalised graph for the proof discussed in the previous
%section. 
This completes the proof of the lemma. 
\fixme{Please, check that
you're happy with this observation.}
\fxnote{jp: I think the new first sentence of the proof should now be sufficient. Therefore, I removed your addition.}
\xbox
\end{proof}
%
...
the following definitions and results are useful.\fixme{Shouldn't the footnote go to the acknowledgements?}\fxnote{jp: done}
