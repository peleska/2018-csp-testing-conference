...
It remains to 
consider the question whether reference processes $\ol Z$ possessing failures $(s,\Sigma)$
may require more test executions for their associated tests $U_F(j)$ than the bound
given for $\pmax$ in (\ref{eq:pmaxcomplexity}), because process states $\ol Z/s$ with  
$\minhits(\ol Z/s)=\varnothing$ allow for test executions entering branch (\ref{eq:ufb}).
To this end, we consider an {\it arbitrary} process  $Z$ represented by its transition graph $G(Z)$ with $p$ nodes. 
...
Now consider a test case ${\ol U}_F(j)$ for $j\in\{ 0,\dots,pq-1 \}$ derived from $G(\ol Z)$
and compare it to $U_F(j)$ derived from $G(Z)$. Let $s$ be a trace of  $\ol Z$ such that
$\#s\le j$ and   $G(\ol Z)/s = \ol v$, so that $s$ is also a trace of ${\ol U}_F(j)$. 
Then we have two cases to distinguish.
\begin{enumerate}
\item If $\#s < j$, then ${\ol U}_F(j)/s$ allows for one additional execution step entering branch 
(\ref{eq:ufb}) of the test, due to construction of $\ol v$. (The other executable branch after
$s$ is (\ref{eq:ufc}) which is also executable for $U_F(j)/s$.)
...
\end{enumerate}
%
The exchange of node $v$ by $\ol v$ can be repeated several times, as long as $\ol v$ is not equivalent to any other node in the successively updated transition graph  $G(\ol Z)$.
As a very rough estimate for the maximal number of additional test executions of ${\ol U}_F(j)/s$
in comparison to $U_F(j)$ we can   assume that $\card{\minhits(v)} = 1$ and that {\it all}
$\card{\Sigma}^k$ traces of length $k < j$ lead to one of the new nodes $\ol v$. As a result,
