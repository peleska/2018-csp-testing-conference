...
(\ref{eq:ufa})  or (\ref{eq:ufb}) after having run through trace
...
\end{enumerate}
%
The number $k$ of traces ${\ol s}_i,\ i=1,\dots,k$ with fixed length $\#{\ol s}_i=\ol j$ is bounded by the maximal number
of traces that can be executed by $(Z\parallel[\Sigma] U_F(\ol j))$, which is bounded by
$\card{\Sigma}^{\ol j}$. If $k$ equals this maximal number, there is no need
...
Summing up over all test cases ${\ol U}_F(j),\ j = \ol j,\dots,pq-1$, for a given 
 $k \le \card{\Sigma}^{\ol j}$, this results in 
an upper bound of
\begin{equation}
 \card{\Sigma}^{\ol j}(pq - \ol j      - 1)
\end{equation}
additional test executions in comparison to the number of executions needed for all tests
of $Z$. The maximal value of this term is reached for $\ol j = pq - 2$: this is the maximal exponent where $(pq - \ol j      - 1)$ is still greater than zero. We conclude that the
maximal increase of test executions to be gained if processes possess failures $(s,\Sigma)$
is of the order $\card{\Sigma}^{pq - 2}$. This can be neglected in comparison to 
the worst case applicable for processes that do not possess any failures $(s,\Sigma)$, because 
formula (\ref{eq:maxO}) shows that the number of test executions required for the latter   is of order $\card{\Sigma}^{pq - 1}$.
