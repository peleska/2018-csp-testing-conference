...
The number of hitting sets in $\minhits(Z/\ol s)$ is 1, for example, if   the
...
So far, the additional executions originating from traces with fixed length $\ol j\in\{0,\dots,pq-2\}$ have been considered. Summing up over all lengths $\ol j$ results in
an upper bound of 
%
\begin{equation}
\sum_{j=0}^{pq-2} n^j(pq - j - 1) = \frac{n^{pq} - npq + pq - 1}{(n-1)^2}\quad\text{with}\ 
n=\card{\Sigma}
\end{equation}
%
additional test executions. This has the order of magnitude $O(n^{pq-2})$. Therefore, the additional test executions won by being able to enter branches (\ref{eq:ufb}) in test executions  can be neglected in comparison to the order of magnitude calculated for $\pmax$ in (\ref{eq:maxO}).  
