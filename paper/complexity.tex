...
I would use $p$ here.} 
\fxnote{jp: ok -- I also had to change this in Theorem~2, where $k$ was used as index from
0 to $pq-1$. I now use $j$ as index for $U_F(j)$, because $p$ is the number of states in $G(P)$.}
section, we
...
\fixme{But doesn't this add to the number of executions?}
\fxnote{jp: should be clear now from the revised test}
...
%
From 
\fixme{Theorem 4 is not
about $conf$. I don't think you have tests for $conf$.}
\fxnote{jp: I rephrased this -- hope that it's better understandable now.}
Theorem~\ref{th:upperboundh} we know that the worst-case bound for $h$
above cannot be further reduced, since the full collection of minimal
hitting sets needs to be checked at the end of each execution of $U_F(j)$. 
...
According 
\fixme{Please, see comments
above about this process.}\fxnote{jp: should be ok now after the above changes and clarifications.}
...
executed for $j = 0,\dots,pq-1$ to guarantee completeness. \fixme{Confusing
use of $p$ and $q$ again.}
\fxnote{jp: fixed}
...
\begin{example}\label{ex:pq}
Consider 
\fxnote{jp: I have updated the example with $P$ and $Q$ as used in the proof of the theorem. This should make thinks more readable.}
...
\fxnote{jp: no -- you are right: we want {\it internal} choice for $P$. With external
choice, this only shows that when testing for trace refinement, the length $pq$ is required.}
...
role.}
\fxnote{jp: should be clear now, since we have explained in Section 3 that only 
the graph product $G(P)\times G(Q)$ is   needed in the proofs.}
%
