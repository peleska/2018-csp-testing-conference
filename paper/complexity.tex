...
\label{eq:pmaxd}
\\
\card{H} & = & \lfloor\frac{n}{s}\rfloor \quad \text{for all $H\in {\cal C}$}
\label{eq:pmaxf}
\end{eqnarray}
\end{lemma}
\begin{proof}
...
\begin{equation}
\label{eq:maxexec}
\card{\Sigma}^{j}\cdot \binom{n}{\lfloor\frac{n}{2}\rfloor}\cdot \lfloor\frac{n}{2}\rfloor,
\end{equation}
%
...
from each of the hitting sets of ${\cal C}$, and each of these hitting sets 
contains $\lfloor\frac{n}{2}\rfloor$ elements.
